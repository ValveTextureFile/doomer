...
static int cli_parse_short(cli_option_t *opt, char **cur)
{
  ...
  while (cli_is_skipchr(*d))
    d++;
  if (*d != '-' || !isalnum(d[1]) || isalnum(d[2]))
    return 0;
...
int cli_usage(int xt)
{
  ...
  if (cli_num_commands > 0)
    fprintf(stderr, " " CLI_STR_COMMANDS);
  if (cli_num_options > 0)
    fprintf(stderr, " " CLI_STR_OPTIONS);
...
static char *cli_get_arg(cli_option_t *opt, char *arg)
{
  ...
  if (((clindx + 1) < cliargc) && !(cliargv[clindx + 1][0] == '-' && cliargv[clindx + 1][1] != '\0'))
...
static int cli_check_short(cli_option_t *opt, char *arg)
{
  ...
  if (arg[cli_reparse_ndx] != opt->optname_short)
    return 0;
...
static int cli_check_long(cli_option_t *opt, char *arg)
{
  ...
  if ((opt->flags & CLI_OPT_COMMAND) && (cli_cmd_found))
    return 0; // cmds can only appear once
  if (strncmp(arg, opt->def + opt->optname_offset, opt->optname_len) != 0)
    return 0;
  if (opt->flags & CLI_OPT_COMMAND)
    cli_cmd_found = 1; // cmds can only appears as first argument (possibly after some '-' options)

  if (arg[opt->optname_len] != '\0' && arg[opt->optname_len] != '=')
    return 0;
...
{
  if (opt->flags & CLI_OPT_FOUND)
    return 0;
  if (opt->flags & (CLI_OPT_FLAG_SHORT | CLI_OPT_FLAG_LONG | CLI_OPT_COMMAND))
    return 0;
  if (!cli_no_flags && (arg[0] == '-' && arg[1] != '\0'))
    return 0;
  cliarg = arg;
  opt->flags |= CLI_OPT_FOUND;
...
{
  if (cli_no_flags)
    return 0; // Already stopped checking for flags
  char *arg = cliargv[clindx];
...
#define cli_options_3(cli_header, cli_arg_cnt, cli_arg_vct)                      \
  {                                                                              \
    ...
    cliargv = cli_arg_vct;                                                       \
    if (cliprogname == NULL)                                                     \
      cliprogname = cli_remove_slash(cliargv[0]);                                \
    if (cli_header != NULL)                                                      \
  ...
  cli_loop:                                                                      \
    for (cliarg = cli_emptystr, cli_opt_found = 0;                               \
      ...
      if (cli_default_errors)                                                    \
        cliusage(CLIEXIT);                                                       \
      else if (cli_double_dash())                                                \
        continue;                                                                \
      else
...
#define cli_opt_2(cli_def, cli_chk)                                                                                                                      \
  ...
  if (cli_opt_found)                                                                                                                                     \
    continue;                                                                                                                                            \
  else if (!((clindx == 0 && cli_opt_define(cli_def, &cli_new_opt, cli_chk)) || (clindx > 0 && (cli_opt_found = cli_check(&cli_new_opt, cli_chk)) > 0))) \
    ;                                                                                                                                                    \
  else
...
#define cli_opt_0()                                           \
  ...
  goto cli_last;                                              \
  cli_last:                                                   \
  if (clindx >= cliargc || cli_opt_found < 0)                 \
    cli_last_check();                                         \
  else                                                        \
    for (cliarg = cliargv[clindx], cli_k = 1; cli_k; cli_k++) \
      if (cli_k == 2)                                         \
      {                                                       \
        clindx++;                                             \
        goto cli_loop;                                        \
